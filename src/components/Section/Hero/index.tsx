...
const HeroSection = ({ lang }: { lang: keyof typeof Text }) => {
  ...
  return (
    ...
    >
      <div className="px-8 z-10 md:mr-80 w-[910px]">
        <motion.h1
          ...
          className="text-3xl sm:text-4xl leading-[110%] tracking-tighter"
        >
        ...
        <motion.h1
          ...
          className="font-black block sm:text-5xl leading-[110%] tracking-tighter" data-value={Text[lang].title.last} ref={titleRef}
        >
        ...
        <ul className="flex translate-y-20 gap-16">{stackIcons.map((icon, index) => (
          <motion.li
            ...
            title={icon.title} key={icon.title} className="rounded-full p-1 flex items-center justify-center">{icon.icon}</motion.li>
        ))}</ul>
